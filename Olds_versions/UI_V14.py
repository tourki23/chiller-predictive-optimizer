...
STYLE_CARD = {
    ...
    "borderRadius": "18px",
    "boxShadow": "0 12px 35px rgba(0,0,0,0.40)" # Ombre légèrement accentuée pour le contraste
}
...
def _make_common_layout(x_range, ui_rev, extra=None):
    layout = dict(
        ...
        plot_bgcolor="rgba(0,0,0,0)", # Plus transparent pour laisser voir le fond sombre
        ...
        # 👇 CORRECTION TOOLTIPS : Fond blanc avec écriture strictement noire
        hoverlabel=dict(
            bgcolor="white",
            font_size=13,
            font_color="black",
...
def build_figures(df_plot, is_opti_active, target_date, x_range, ref_m1, ref_m2):
    ...
    if df_plot.empty:
        fig_meteo = go.Figure(
            layout=_make_common_layout(
                x_range,
                ui_rev,
                {'title': dict(text=f"Météo : {target_date.strftime('%d/%m/%Y')}", x=0.5)}
            )
        )
        ...
        fig_temp = go.Figure(
            layout=_make_common_layout(
                x_range,
                ui_rev,
                {'title': dict(text=f"Simulation Thermique LSTM — Modèle : {ref_m1}", x=0.01)}
            )
        )
        ...
        fig_pwr = go.Figure(
            layout=_make_common_layout(
                x_range,
                ui_rev,
                {'title': dict(text=f"Consommation Énergétique XGBoost — Modèle : {ref_m2}", x=0.01)}
            )
        )
    ...
    traces_temp = [
        ...
        go.Scatter(
            ...
            mode='lines',
            name="T° Réelle",
            line=dict(color="#4A84D4", width=2)
        ),
        go.Scatter(
            x=on_reel['timestamp'],
            y=on_reel['T_int'],
            mode='markers',
            marker=dict(color='#A4E063', size=10),
            name='ON (Réel)'
        ),
        go.Scatter(
            x=off_reel['timestamp'],
            y=off_reel['T_int'],
            mode='markers',
            marker=dict(color='#D65F5F', size=10),
            name='OFF (Réel)'
        ),
        go.Scatter(
            x=df_plot['timestamp'],
            y=df_plot['t_int_pred'],
            mode='lines',
            name="T° IA",
    ...
    fig_pwr = go.Figure(
        data=[
            go.Scatter(
                x=df_plot['timestamp'],
                y=df_plot['Puissance_Elec_kW'],
                mode='lines',
                name="Réel",
                line=dict(color="#f89406")
            ),
            go.Scatter(
                x=df_plot['timestamp'],
                y=df_plot['pwr_pred_ia'],
                mode='lines',
                name=f"IA: {ref_m2}",
                line=dict(color=couleur_ia, dash='dash')
            ),
...
def build_kpi(df_plot, is_opti_active):
    ...
    delta = total_opt - total_reel
    delta_text = f"{delta:.1f} kWh" if delta < 0 else f"+{delta:.1f} kWh"

    def kpi_item(label, value, color, subtitle=None):
        return html.Div([
    ...
    dbc.Col([
        dbc.Card([
            ...
            dbc.CardBody([
                ...
                # 👇 CORRECTION BOUTON PLAY : Bleu profond & Pro
                dbc.Button(
                    "▶️ Play / Pause",
                    id="btn-play",
                    color="primary", 
                ...
                # 👇 CORRECTION BOUTON OPTI : Synchronisé avec la couleur globale 'couleur_opt'
                dbc.Button(
        ...
        dbc.Col([
            html.Div([
                html.Div([
                    ...
                    # 👇 Side-by-side title and subtitle section. Removed badges.
                    html.Div([
                        html.H2(
                        ...
                        html.Div(
                            ...
                            style={
                                ...
                                "marginBottom": "0px", # Remove extra margin
                            }
                        )
                    ], style={"display": "flex", "alignItems": "center"}), # flex container for side-by-side title/subtitle

                ], style={
                    "display": "flex",
                    "alignItems": "center"
                }),
                # Inner Div of title box is updated. Badges Div is removed.
            ], style={
                "padding": "22px 26px",
...
def update_everything(
    ...
    if current_idx == 0 and not is_playing:
        ...
        # 👇 CORRECTION ALERTE TERMINÉE : Ardoise Neutre (plus de violet)
        sim_status = dbc.Alert(
            "⏹️ Terminée",
            color="secondary", 
...
def update_opti_table(active_tab, phys_name, n_int):
    ...
    try:
        ...
        # Rounding logic added: floats to 2 decimals, special slopes to 4 decimals.
        ...
        table = dash_table.DataTable(
            ...
            #old style_header=STYLE_TABLE_HEADER,
            # 👇 Modified for consistent factoring: size multiplier 2 (baseline 13px e.g. "26px"). ochre color ALL as exhausted list.
            style_header={**STYLE_TABLE_HEADER, 'fontSize': '26px'},
            style_header_conditional=[
                {'if': {'column_id': c}, 'color': ACCENT_ORANGE} for c in df_opti.columns
            ],
...
def update_perf_tab(active_tab, phys_name):
    ...
    img_val = get_b64_image(
        f"{base_path}/validation_vs_test_{suffix}.png"
    )

    img_feat = get_b64_image(
        f"{base_path}/feature_importance_lstm_{suffix}.png"
    )

    img_learn = get_b64_image(
        f"{base_path}/learning_curve_{suffix}.png"
    )
    ...
    images_row = dbc.Row([
        dbc.Col(
            ...
            ) if img_val else html.P(
                "Graphique Validation non trouvé.",
        ...
        dbc.Col(
            html.Img(
                ...
                style=base_style.copy(),
                className="img-fluid border border-secondary"
            ) if img_feat else html.P(
                "Graphique Feature Importance non trouvé.",
                className="text-muted"
            ),
            width=4
        ),
        ...
        dbc.Col(
            html.Img(
                ...
                style=base_style.copy(),
                className="img-fluid border border-secondary"
            ) if img_learn else html.P(
                "Graphique Learning Curve non trouvé.",
                className="text-muted"
            ),
            width=4
        ),
    ...
    try:
        ...
        df_metrics = df_metrics[
            ~df_metrics['Nom du Modèle'].str.contains(
                'Modèle',
                case=False,
                na=False
            )
        ...
        metrics_table = dash_table.DataTable(
            ...
            #old style_header=STYLE_TABLE_HEADER,
            # 👇 Modified for consistent factoring: size multiplier 2 (baseline 13px e.g. "26px"). ochre color headers of 'Nom du Modèle' and 'Cible'.
            style_header={**STYLE_TABLE_HEADER, 'fontSize': '26px'},
            style_header_conditional=[
                {'if': {'column_id': 'Nom du Modèle'}, 'color': ACCENT_ORANGE},
                {'if': {'column_id': 'Cible'}, 'color': ACCENT_ORANGE},
            ],
            style_data=STYLE_TABLE_DATA,
            style_cell=STYLE_TABLE_CELL,
            style_table=STYLE_TABLE,
            style_data_conditional=[
                {
                    'if': {'filter_query': '{Nom du Modèle} contains "LSTM"'},
                    'backgroundColor': 'rgba(91, 212, 242, 0.15)'
                },
                {
                    'if': {'filter_query': '{Nom du Modèle} contains "XGB"'},
                    'backgroundColor': 'rgba(46, 204, 113, 0.15)' # Ajusté pour correspondre au nouveau vert
                },
            ]
        )
        ...
        metrics_table = html.Div(
            f"Fichier metrics_summary.csv introuvable ou illisible ({e}).",
            className="text-danger"
        )

    return html.Div([
        html.H3(
            f"📈 Performances des Modèles — Variante : {phys_name}",
            style={"color": TEXT_LIGHT, "fontWeight": "800"},
            className="mb-2"
        ),

        html.Div(
            "Visualisation des métriques de validation, importance des variables et courbes d’apprentissage.",
            style={"color": TEXT_MUTED},
            className="mb-4"
        ),
        ...
        html.H3(
            "📋 Tableau Récapitulatif des Métriques MLOps",
            style={"color": TEXT_LIGHT, "fontWeight": "800"},
            className="mb-2"
        ),

        html.Div(
            "Comparaison synthétique des performances des variantes LSTM et XGBoost.",
            style={"color": TEXT_MUTED},
            className="mb-4"
        ),

        dbc.Row(
            dbc.Col(metrics_table, width=10)
        )
    ])
...
def update_sim_table(active_tab, daily_data, picked_date):
    ...
    # Float rounding logic remains correct as per request 4.
    ...
    table = dash_table.DataTable(
        ...
        style_table=STYLE_TABLE,
        # old style_header=STYLE_TABLE_HEADER,
        # 👇 Modified for consistent factoring: size multiplier 2 (baseline 13px e.g. "26px"). ochre color headers of primary data columns.
        style_header={**STYLE_TABLE_HEADER, 'fontSize': '26px'},
        style_header_conditional=[
            {'if': {'column_id': 'timestamp'}, 'color': ACCENT_ORANGE},
            {'if': {'column_id': 'T_ext'}, 'color': ACCENT_ORANGE},
            {'if': {'column_id': 'T_int'}, 'color': ACCENT_ORANGE},
            {'if': {'column_id': 'Puissance_Elec_kW'}, 'color': ACCENT_ORANGE},
        ],
        style_data=STYLE_TABLE_DATA,
        style_cell=STYLE_TABLE_CELL,
        style_as_list_view=True
