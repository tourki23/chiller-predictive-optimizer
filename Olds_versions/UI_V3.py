...
print("=" * 50)
print("🌐 JUMEAU NUMÉRIQUE V16 — Architecture Réactive & Stable")
print("=" * 50)
...
# 👇 VOICI LA PARTIE QUI AVAIT DISPARU 👇
_day_cache = {}
...
def build_kpi(df_plot, is_opti_active):
    if len(df_plot) < 2: return html.Div("Initialisation...", className="text-muted mt-3")
...
app.layout = dbc.Container([
    # STORES INVISIBLES
    dcc.Store(id='store-daily-data'), dcc.Store(id='store-idx', data=2),
    ...
    # L'horloge est DÉSACTIVÉE par défaut pour ne pas polluer le réseau
    dcc.Interval(id='interval', interval=INTERVAL_MS, n_intervals=0, disabled=True)
], fluid=True)


# ===========================================================
# CALLBACK 1 : Chargement des Modèles
# ===========================================================
@app.callback(
...
def update_models_registry(phys_name, ener_name):
    ...
    try:
        ...
        return dbc.Badge("Modèles chargés", color="success", className="w-100 p-2 mb-2"), time.time()
    except Exception as e:
        return dbc.Badge(f"Erreur: {str(e)}", color="danger", className="w-100 p-2 mb-2"), dash.no_update


# ===========================================================
# CALLBACK 2 : Calcul des données
# ===========================================================
@app.callback(
...
# ===========================================================
# CALLBACK 3 : Le Cerveau Central de l'UI
# ===========================================================
@app.callback(
    [Output('graph-meteo', 'figure'), Output('graph-temperature', 'figure'), Output('graph-puissance', 'figure'),
     ...
     Output('sim-status-notification', 'children'), Output('interval', 'disabled')],
    [Input('interval', 'n_intervals'), Input('btn-play', 'n_clicks'),
     Input('btn-reset', 'n_clicks'), Input('store-daily-data', 'data'),
     Input('store-opti-state', 'data')], # <-- NOUVEAU: Écoute directement l'état de l'opti
...
def update_everything(n_int, n_play, n_reset, data, is_opti, current_idx, is_playing):
    ...
    # Au premier lancement, on initialise proprement
    if not ctx.triggered:
        trigger_id = 'store-daily-data'
    else:
        trigger_id = ctx.triggered[0]['prop_id'].split('.')[0]
    
    # 1. Gestion stricte des actions temporelles
    if trigger_id == 'store-daily-data':
        current_idx, is_playing = 2, False
    ...
    # Si trigger_id == 'store-opti-state', l'index et l'état play ne changent pas, on met juste à jour l'image.
...
# ===========================================================
# CALLBACK 4 : Le Gestionnaire Exclusif de l'Optimisation
# ===========================================================
@app.callback(
...
def toggle_optimisation(n_clicks, is_opti_active):
    ...
    # On renvoie l'état pour le store, et True/False pour l'outline du bouton
...
if __name__ == '__main__':
    # On force le chargement initial en appelant la fonction une première fois
