...
def build_figures(df_plot, is_opti_active, target_date, x_range, ref_m1, ref_m2):
    ...
    # Météo en rouge
    fig_meteo = go.Figure(data=[go.Scatter(x=df_plot['timestamp'], y=df_plot['T_ext'], mode='lines', name="T° Extérieure", line=dict(color="#FF4444"))],
                          layout=_make_common_layout(x_range, ui_rev, {'title': dict(text=f"Météo : {target_date.strftime('%d/%m/%Y')}", x=0.5)}))

    # NOUVEAU : showlegend=False pour les marqueurs afin d'alléger la légende
    traces_temp = [
        go.Scatter(x=df_plot['timestamp'], y=df_plot['T_int'], mode='lines', name="🔵 Réel", line=dict(color="#2485DB", width=2)),
        go.Scatter(x=on_reel['timestamp'], y=on_reel['T_int'], mode='markers', marker=dict(color='#00FF00', size=11), name='🟢 ON Réel', showlegend=False),
        go.Scatter(x=off_reel['timestamp'], y=off_reel['T_int'], mode='markers', marker=dict(color='#FF4444', size=11), name='🔴 OFF Réel', showlegend=False),
    ...
    if is_opti_active:
        ...
        traces_temp += [
            ...
            go.Scatter(x=on_opt['timestamp'], y=on_opt['t_int_opt'], mode='markers', marker=dict(color='#00FF00', size=14, symbol='star', line=dict(color='white', width=1)), name='★ ON Opti', showlegend=False),
            go.Scatter(x=off_opt['timestamp'], y=off_opt['t_int_opt'], mode='markers', marker=dict(color='#FF4444', size=14, symbol='star', line=dict(color='white', width=1)), name='★ OFF Opti', showlegend=False),
        ]
...
def update_perf_tab(active_tab, phys_name):
    ...
    # 1. Extraction du suffixe du modèle pour cibler le bon dossier
    ...
    # 2. Chargement des images PNG en Base64
    ...
    images_row = dbc.Row([
        dbc.Col(html.Img(src=img_val, className="img-fluid rounded border border-secondary") if img_val else html.P("Graphique Validation non trouvé.", className="text-muted"), width=4),
        dbc.Col(html.Img(src=img_feat, className="img-fluid rounded border border-secondary") if img_feat else html.P("Graphique Feature Importance non trouvé.", className="text-muted"), width=4),
        dbc.Col(html.Img(src=img_learn, className="img-fluid rounded border border-secondary") if img_learn else html.P("Graphique Learning Curve non trouvé.", className="text-muted"), width=4),
    ], className="mb-5")

    # 3. Chargement et formatage du fichier CSV global
    try:
        ...
        metrics_table = dash_table.DataTable(
            ...
            style_data_conditional=[
                {'if': {'filter_query': '{Nom du Modèle} contains "LSTM"'}, 'backgroundColor': 'rgba(91, 212, 242, 0.15)'}, # Couleur IA (Cyan)
                {'if': {'filter_query': '{Nom du Modèle} contains "XGB"'}, 'backgroundColor': 'rgba(141, 246, 75, 0.15)'},  # Couleur Opti (Vert fluo)
            ]
        )
    except Exception as e:
