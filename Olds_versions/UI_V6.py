...
print("=" * 50)
print("🌐 JUMEAU NUMÉRIQUE V18 — Sync Parfaite & Zéro Globals UI")
print("=" * 50)
...
def build_figures(df_plot, is_opti_active, target_date, x_range, ref_m1, ref_m2):
    ...
    # 🎨 Courbe réelle de consommation en orange (#f89406)
...
@app.callback(
    ...
    [State('store-idx', 'data'), State('store-play-state', 'data'),
     State('dropdown-physicien', 'value'), State('dropdown-energetique', 'value')] # <-- LES TITRES SONT LUS DIRECTEMENT ICI !
)
def update_everything(n_int, n_play, n_reset, data, is_opti, current_idx, is_playing, phys_name, ener_name):
    ...
    if not ctx.triggered:
        ...
        # LA SOLUTION ANTI-CONFLIT : On lit TOUS les déclencheurs
        trigger_ids = [t['prop_id'].split('.')[0] for t in ctx.triggered]
    
    # LA PRIORITÉ ABSOLUE AUX ACTIONS DE L'UTILISATEUR
    ...
    # LE DESSIN DU GRAPHIQUE AVEC LES BONS TITRES LUS DEPUIS L'ÉTAT DU MENU DÉROULANT
...
# LANCEMENT
# ===========================================================
# Chargement du modèle de base sans toucher aux UI elements
