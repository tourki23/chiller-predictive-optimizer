...
def build_kpi(df_plot, is_opti_active):
    if df_plot.empty:
        ...
        total_ia_mn = (df_plot['Etat_Compresseur'] == 1).sum() * 10 # Same as real
    ...
    return html.Div([
        kpi_row_item(
            ...
            f"{total_reel_mn} mn",
            ACCENT_ORANGE, # Energy color
            ACCENT_ORANGE, # Time color
            "Consommation réelle"
        ),
...
tab_supervision_content = dbc.Row([
    dbc.Col([
        ...
        dbc.Card([
            # New flex header for KPIs to separate Energy from Time.
            dbc.CardHeader(
                html.Div([
                    html.Span("⚡ Energy KPIs"),
                    html.Span("⏰ Time", style={'marginLeft': 'auto'})
    ...
    dbc.Row([
        dbc.Col([
            html.Div([
                ...
                # This section has been removed at user request.
            ], style={
                "padding": "22px 26px",
...
def update_sim_table(active_tab, daily_data, picked_date):
    ...
    table = dash_table.DataTable(
        ...
        page_size=20,
        style_table=STYLE_TABLE,
        style_header={**STYLE_TABLE_HEADER, 'fontSize': '26px'}, # Doubled font size
        style_header_conditional=[
