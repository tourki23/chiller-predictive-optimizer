...
# specific header style globally uses no font size specified, doubling it globally for standard globally
STYLE_TABLE_HEADER = {
...
# GLOBAL cell size standard standard globally for standard standard globally values standard globally (user says labels too small not content labels of other tables)
STYLE_TABLE_CELL = {
    'textAlign': 'center',
    'padding': '10px',
    'fontFamily': 'Inter, Segoe UI, Arial',
    'fontSize': '13px' # GLOBAL Standard font size for value standard globally (image 4 close up of result headers visuals results image headers close up)
}
    ...
    dbc.Row([
        dbc.Col([
            html.Div([
                ...
                # 👇 SIDE-BY-SIDE OPTIMIZATION SECTIONS - Harmonized Colors
                html.Div([
                    dbc.Row([
                        dbc.Col([
                            html.Div([
                                html.Div("Usine — 24h (Consommation)", style={
                                    "color": TEXT_MUTED,
                                    "fontSize": "11px",
                                    "textTransform": "uppercase"
                                }),
                                html.Span("❄️ Usine réelle", style={
                                    "color": TEXT_LIGHT,
                                    "fontWeight": "600",
                                    "fontSize": "13px"
                                })
                        ...
                        dbc.Col([
                            html.Div([
                                html.Div("Optimisation (Demande)", style={
                                    "color": TEXT_MUTED,
                                    "fontSize": "11px",
                                    "textTransform": "uppercase"
                                }),
                                html.Span("❄️ Optimisation IA", style={
                                    "color": couleur_opt, # Vert harmonisé
                                    "fontWeight": "600",
                                    "fontSize": "13px"
                                })
                            ])
                        ...
                        "marginTop": "22px",
                        "paddingTop": "15px",
                        "borderTop": f"1px solid {BORDER_SOFT}"
                    }),
                ])
            ], style={
                "padding": "22px 26px",
...
def update_opti_table(active_tab, phys_name, n_int):
    ...
    try:
        ...
        # Arrondi à 2 chiffres (et 4 pour les pentes)
        ...
        # Les colonnes à mettre en jaune ocre pour cette table
        target_ochre_headers = ['timestamp', 't_int_opt', 'etat_opt', 'pwr_opt']
...
def update_sim_table(active_tab, daily_data, picked_date):
    ...
    # Exclusion exhaustive traitement : application boucle color ocre systématique (synchronisation)
