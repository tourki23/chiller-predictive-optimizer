...
logging.basicConfig(
    filename='app_logs/app_digital_twin.log', # <--- On ajoute le chemin du dossier ici
    level=logging.INFO, 
...
def build_kpi(df_plot, is_opti_active):
    if df_plot.empty:
        ...
        # Calcul du temps de fonctionnement (chaque ligne = 10 minutes)
        total_reel_mn = (df_plot['Etat_Compresseur'] == 1).sum() * 10
        total_ia_mn = (df_plot['Etat_Compresseur'] == 1).sum() * 10 # Même chose que réel car la prédiction suit l'état
